# tijos.runtime.deviceaccess.TiI2CMaster - 双向二线制同步串行总线，主机模式
...
TiJOS JDK提供TiI2CMaster来支持I2C主机功能，其将I2C Master按照port分组，共支持4组，组号范围0-3，即：port0-port3；每组都支持指定模式的单独设置。
...
| int read(byte[] buf, int offset, int len) | 从从机读数据                                   |
| int write(byte[] buf, int offset, int len) | 向从机写数据                                   |

TiI2CMaster类中他方法的技术说明请参考TiJOS JDK Runtime说明文档。
